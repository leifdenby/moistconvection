...
class RICO(BaseCase):
    ...
    def lh_surf_flux(self):
        ...
        NB: forcing was extracted from quasi steady-state in LES since actual
        forcing is prescribed using bulk aero-dynamic formulation (e.g. depends
        on tangential wind speed)
        """
        warnings.warn("""forcing was extracted from quasi steady-state in LES
        since actual forcing is prescribed using bulk aero-dynamic formulation
        (e.g. depends on tangential wind speed)""")
    ...
    def __str__(self):
        ...
        return super(RICO, self).__str__() + ext
